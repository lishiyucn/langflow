...
Notion 会议笔记代理是一个 AI 驱动的工具，可以自动处理会议记录并更新您的 Notion 工作区。它从您的会议中识别任务、行动项目和关键点，然后在 Notion 中创建新任务或更新现有任务，无需手动输入。
...
- [下载流会议代理流](./Meeting_Notes_Agent.json)（下载链接）

:::important
在使用此流程之前，请确保您已从 Notion 和 OpenAI 获取必要的 API 密钥。这些密钥对于流程的正常运行至关重要。请安全保存并不要公开分享。
:::
...
2. 将上述组件添加到您的流程画布中，或下载 [流会议代理流](./Meeting_Notes_Agent.json)（下载链接）并将 JSON 文件**导入**到 Langflow 中。
3. 按流程图所示连接组件。
...
为了获得最佳结果，请使用详细的会议记录。输出质量取决于提供的输入的全面性。
