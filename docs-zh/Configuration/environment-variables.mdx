...
Langflow 从以下来源识别[支持的环境变量](#supported-variables)：

- 您在终端中设置的环境变量。
- 您使用 Langflow CLI 中的 `--env-file` 选项从 `.env` 文件导入的环境变量。

您可以选择专门使用一个来源，或者将两个来源一起使用。
如果您选择将两个来源一起使用，请注意从 `.env` 文件导入的环境变量比您在终端中设置的环境变量具有更高的[优先级](#precedence)。

### 在终端中设置环境变量 {#configure-variables-terminal}
...
当您启动 Langflow 时，它会查找您在终端中设置的环境变量。
如果它检测到支持的环境变量，则会自动采用指定的值，遵循[优先级规则](#precedence)。

### 从 .env 文件导入环境变量 {#configure-variables-env-file}

1. 如果 Langflow 正在运行，请退出 Langflow。

2. 创建 `.env` 文件，然后在您首选的编辑器中打开它。

3. 在 `.env` 文件中定义 [Langflow 环境变量](#supported-variables)。例如：
   ...
   有关其他示例，请参阅 Langflow 仓库中的 [`.env.example`](https://github.com/langflow-ai/langflow/blob/main/.env.example) 文件。
...
在启动时，Langflow 从您的 `.env` 文件以及您[在终端中设置](#configure-variables-terminal)的任何环境变量导入环境变量，并采用它们指定的值。

## 优先级 {#precedence}

您可以在终端、`.env` 中以及使用 [Langflow CLI 选项](./configuration-cli.mdx) 设置 Langflow 环境变量。
...
例如，如果您在 `.env` 和终端中都设置了 `LANGFLOW_PORT`，则 Langflow 使用 `.env` 中的值。
同样，如果您使用 `--port` 运行 Langflow CLI 命令，则 Langflow 使用该端口号而不是 `.env` 中的 `LANGFLOW_PORT`。

## 支持的环境变量 {#supported-variables}
...
| 变量 | 格式 | 默认值 | 描述 |
|----------|--------|---------|-------------|
| <Link id="DO_NOT_TRACK"/>DO_NOT_TRACK | Boolean | `false` | 如果启用此选项，Langflow 不会跟踪遥测数据。 |
| <Link id="LANGFLOW_AUTO_LOGIN"/><span class="env-prefix">LANGFLOW_</span>AUTO_LOGIN | Boolean | `true` | 为 Langflow 启用自动登录。设置为 `false` 可禁用自动登录并需要登录表单来登录 Langflow UI。设置为 `false` 需要设置 [`LANGFLOW_SUPERUSER`](#LANGFLOW_SUPERUSER) 和 [`LANGFLOW_SUPERUSER_PASSWORD`](environment-variables.mdx#LANGFLOW_SUPERUSER_PASSWORD)。有关更多信息，请参阅[身份验证](/configuration-authentication)。 |
| <Link id="LANGFLOW_AUTO_SAVING"/><span class="env-prefix">LANGFLOW_</span>AUTO_SAVING | Boolean | `true` | 启用 flow 自动保存。<br/>参阅 [`--auto-saving` 选项](./configuration-cli.mdx#run-auto-saving)。 |
| <Link id="LANGFLOW_AUTO_SAVING_INTERVAL"/><span class="env-prefix">LANGFLOW_</span>AUTO_SAVING_INTERVAL | Integer | `1000` | 设置 flow 自动保存的间隔（毫秒）。<br/>参阅 [`--auto-saving-interval` 选项](./configuration-cli.mdx#run-auto-saving-interval)。 |
| <Link id="LANGFLOW_BACKEND_ONLY"/><span class="env-prefix">LANGFLOW_</span>BACKEND_ONLY | Boolean | `false` | 仅运行 Langflow 的后端服务器（无前端）。<br/>参阅 [`--backend-only` 选项](./configuration-cli.mdx#run-backend-only)。 |
