...
Langflow 组件被设计为接受和产生特定类型的输入和输出。
输入和输出数据类型定义了组件间信息的结构和流向。
理解这些结构有助于您构建提供有效输入并正确预期输出格式的应用程序。

[组件端口](/concepts-components#component-ports)表示每个组件可以发送和接收的数据类型。
某些数据类型从它们所附加的字段中就很明显；例如，**System Message** 字段接受[消息数据](#message)。
[端口颜色](/concepts-components#port-colors)也表示端口的数据类型。
例如 **Data** 端口，由 <Icon name="Circle" size="16" aria-label="Red data port" style={{ color: '#dc2626', fill: '#dc2626' }} /> 表示，可以接受或发出[结构化数据对象](#data)。

构建 flow 时，将输出端口连接到相同类型（颜色）的输入端口，以在两个组件之间传输该类型的数据。

:::tip
* 在可视化编辑器中，将鼠标悬停在端口上可查看该端口的连接详细信息。
...
**Data** 端口 <Icon name="Circle" size="16" aria-label="Red data port" style={{ color: '#dc2626', fill: '#dc2626' }} /> 接受或产生 `Data` 类型，这是一个结构化数据对象，就像您可能发送到 API 的 JSON 载荷。
此数据类型用于在组件之间传递键值对，如用户配置文件、设置或其他结构化信息。

`Data` 对象包括一个主要文本字段（由 `text_key` 指示）和附加元数据。

### 模式和属性

模式在 [`data.py`](https://github.com/langflow-ai/langflow/blob/main/src/backend/base/langflow/schema/data.py) 中定义。

以下属性可用：

- `data`: 存储键值对的字典。
- `text_key`: `data` 中被认为是主要文本值的键。
- `default_value`: 如果 `text_key` 缺失的回退值。默认 `text_key` 是 `"text"`。

### 数据结构

`Data` 对象在 `.data` 属性中存储键值对，其中每个键都是字段名，其值可以是任何支持的数据类型。`text_key` 告诉 Langflow 数据字典中的哪个键是该对象的主要文本值。
