...
远程托管的 Langflow 部署可在托管服务设置的域名访问，例如 `http://IP_OR_DNS/api` 或 `http://IP_OR_DNS:LANGFLOW_PORT/api`。
...
要验证 Langflow API 请求，请在 `x-api-key` 头或查询参数中提供 Langflow API 密钥。
有关更多信息，请参阅 [API 密钥](/configuration-api-keys)。

与任何 API 一样，请遵循行业最佳实践来存储和引用敏感凭据。
例如，您可以为 API 密钥[设置环境变量](#set-environment-variables)，然后在 API 请求中引用这些环境变量。
...
然后，要运行您的流程，您需要调用 `POST /v1/run/$FLOW_ID`，并在请求主体中包含可选的运行参数。
...
如果请求失败或有意外结果，请确保您的端点路径具有正确的版本。

## 设置环境变量

您可以将常用值存储在环境变量中，以便于重用、简化令牌轮换并安全地引用敏感值。

您可以使用任何您喜欢的方法来设置环境变量，如 `export`、`.env`、`zshrc` 或 `.curlrc`。
...
您可以从 [**API 访问** 窗格](/concepts-publish#api-access)、流程的 URL 以及 [`GET /flows`](/api-flows#read-flows) 中检索流程 ID。
